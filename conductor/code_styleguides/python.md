...
- **Linting & Formatting:** `ruff` MUST be used for all Python code. 
- **Pre-commit/Pre-build:** Run `ruff check --fix .` and `ruff format .` before any commit or build to ensure valid indentation and consistent style.
- **Strict Mode:** Indentation errors or unused imports are considered critical failures.
